...
public abstract class FurnaceBaseTileEntity extends LockableTileEntity implements ISidedInventory, IRecipeHolder, IRecipeHelperPopulator, ITickableTileEntity {
	...
	protected FurnaceBaseTileEntity(TileEntityType<?> typeIn, final IRecipeType<? extends AbstractCookingRecipe> recipeTypeIn) {
	...
	public void tick() {
		...
		boolean flag1 = false;
		if (this.isBurning()) { // If burning
			
			--this.burnTime;    //Subtract tick from burn time
		}

		if (!this.world.isRemote) { //If server
			
			ItemStack fuel = this.items.get(1); 
			
			if (this.isBurning() || !fuel.isEmpty() && !this.items.get(0).isEmpty()) { //If is burning or itemstack is not empty and first item in items is not empty
				
				IRecipe<?> irecipe = this.world.getRecipeManager().getRecipe((IRecipeType<AbstractCookingRecipe>)this.recipeType, this, this.world).orElse(null); //Get recipe
				
				
				if (!this.isBurning() && this.canSmelt(irecipe)) { //If is not burning and can smelt
			...
			if (flag != this.isBurning()) {
				flag1 = true;
				this.world.setBlockState(this.pos, this.world.getBlockState(this.pos).with(FurnaceBaseBlock.LIT, Boolean.valueOf(this.isBurning())), 3);
			}
		}
